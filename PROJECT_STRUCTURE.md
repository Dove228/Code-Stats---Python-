...
│   ├── __main__.py          # CLI命令行入口
│   │
│   ├── core/                # 核心模块
│   │   ├── __init__.py
│   │   ├── analyzer.py      # 代码分析器
│   │   ├── complexity.py    # 复杂度分析
│   │   ├── languages.py      # 语言定义（30+语言）
│   │   ├── scanner.py       # 文件扫描器
│   │   └── stats.py         # 统计数据结构
│   │
│   ├── exporters/           # 导出器模块
...
├── Python文件: 16个
├── Markdown文件: 3个
└── 配置文件: 3个 (.gitignore, LICENSE, pyproject.toml, requirements.txt)
```
...
- 添加新导出格式：在`exporters/`中添加新的Exporter类
- 添加新功能：在`utils/`中添加新的工具类
- 添加测试：在`tests/`中添加测试文件
